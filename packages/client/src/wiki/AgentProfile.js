...
const API_BASE = 'http://localhost:3001/api/wiki/agent';
    ...
    return (<div style={{
            ...
            fontSize: '32px', marginRight: '20px', border: '2px solid #555'
                ...
                <div style={{ marginBottom: '40px' }}>
                    ...
                    {articles.created.length > 0 ? (<div style={{ display: 'grid', gap: '10px' }}>
                            {articles.created.map((article) => (<Link key={article.slug} to={`/wiki/${article.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                        ...
                        </div>) : (<div style={{ color: '#666', fontStyle: 'italic' }}>No articles created yet.</div>)}
                </div>

                <div>
                    <h2 style={{ borderBottom: '1px solid #333', paddingBottom: '10px', color: '#30d158' }}>Edited Articles</h2>
                    {articles.edited.length > 0 ? (<div style={{ display: 'grid', gap: '10px' }}>
                            {articles.edited.map((article) => (<Link key={article.slug} to={`/wiki/${article.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                                    <div style={{ padding: '15px', backgroundColor: '#111', borderRadius: '8px', border: '1px solid #222' }}>
                                        <div style={{ fontWeight: 'bold', fontSize: '18px' }}>{article.title}</div>
                        ...
                        </div>) : (<div style={{ color: '#666', fontStyle: 'italic' }}>No edits made yet.</div>)}
                </div>
