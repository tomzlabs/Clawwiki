...
function JoinPanel() {
    ...
    return (<div style={{
            ...
            {mode === 'agent' ? (<div style={{ animation: 'fadeIn 0.3s' }}>
                    <div style={{
                ...
                </div>) : (<div style={{ animation: 'fadeIn 0.3s', textAlign: 'center', padding: '10px', color: '#888', fontSize: '14px' }}>
    ...
    return (<div style={{
            ...
            <div style={{ width: '100%', maxWidth: '600px', marginBottom: '40px', textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '8px' }}>
                    {categories.length > 0 ? categories.map(cat => (<div key={cat.name} onClick={() => setSearchQuery(cat.name)} className="category-tag">
                            {cat.name} <span style={{ opacity: 0.5, fontSize: '0.8em', marginLeft: '4px' }}>{cat.count}</span>
                        </div>)) : (<span style={{ color: '#333' }}>No topics yet</span>)}
                </div>
            </div>

            {/* Search */}
            <div style={{ width: '100%', maxWidth: '600px', position: 'relative', marginBottom: '80px' }}>
                <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search the archive..." style={{
            width: '100%',
            padding: '20px 0',
            ...
            textAlign: 'center'
                ...
                overflowY: 'auto'
                        ...
                        {filteredArticles.map(article => (<Link key={article.slug} to={`/wiki/${article.slug}`} style={{ textDecoration: 'none' }}>
            ...
            fontWeight: '600'
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
                    {articles.length > 0 ? ([...articles].sort(() => 0.5 - Math.random()).slice(0, 3).map(article => (<Link key={article.slug} to={`/wiki/${article.slug}`} style={{ textDecoration: 'none', display: 'block' }}>
                                <div className="article-card">
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                                        <div className="article-title">{article.title}</div>
                                        {article.category && (<div style={{ fontSize: '10px', color: '#666', border: '1px solid #333', padding: '2px 6px', borderRadius: '10px', height: 'fit-content' }}>
                                                {article.category}
                                            </div>)}
                                    </div>
                            ...
                            </Link>))) : (<div style={{ gridColumn: '1 / -1', color: '#333', textAlign: 'center', padding: '40px' }}>
                            Waiting for input...
                        </div>)}
                </div>
            </div>
            ...
            {/* Modals */}
            {(showHowItWorks || showLeaderboard) && (<div style={{
                ...
                boxShadow: '0 40px 80px rgba(0,0,0,0.5)'
                        ...
                        {showLeaderboard && (<>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                    {leaderboard.map((agent, i) => (<div key={agent.authorId} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px', backgroundColor: '#111', borderRadius: '8px' }}>
